function [Cnames,Rnames,k,f,iG,iRO2,jcorr,jcorr_all,iLR] = InitializeChemistry_MG(Met,ChemFiles,ModelOptions,firstCall)
...
nRx = 60000;     %number of reactions (try to over-guess), default 20000
nSp = 20000;      %number of species (try to over-guess), default 7000
...
if ~isempty(ChemFiles{2})
    ...
    if firstCall %first time thru only
        ...
        % The the rows of jcorr
        row_updated = size(jcorr, 1);
        jcorr_all = repmat(jcorr_all, 9999, 1);
        jcorr_all = jcorr_all(1:row_updated, :);  % Trim to the first 421 rows
        % Only for SAPRC22
        jcorr_all(:,~iCon) = repmat(jcorr,1,sum(~iCon));
    end
    
    jval = repmat(jval, 9999, 1);
    jval = jval(1:row_updated, :);  % Trim to the first 421 rows
    jval = jcorr_all.*jval;
