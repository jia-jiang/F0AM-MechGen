...
Created by Jia Jiang, 2024/12/8, used for windows
"""
...
def output_multi_gen_files(putty: Any, compound: str, minyld: float, output_types: List[str], output_dir: str, timewait: int) -> None:
    """
    ...
    for file_type in output_types:
        ...
        elif file_type in ["Summary", "Cmpdinfo", "Products", "Formedby", "Unirxns"]:
        ...
        file_new = os.path.join(output_dir, f"MG-{compound}_{file_type}_{minyld}.dat")
...
def main() -> None:
    ...
    OPTION_MULTI_ON = False
    OPTION_MULTI_LIST = {
        "MinYld": 0.001,     # Default 0.0001, between 0~0.5
        "RxnHours": 6,      # Default 6
    }

    # Operation switches
    BUILD_MULTI_ON  = True    # Build compound?
    REACT_MULTI_ON  = True    # React the compound?
    ...
    try:
        ...
        if OUTPUT_MULTI_ON:
            minyld = OPTION_MULTI_LIST["MinYld"]
            if os.path.exists(OUTPUT_DIR):
                compound, _ = list(DICT_COMPOUNDS.items())[0]
                logging.info(f"Outputting multi-generation files for '{compound}'")
                output_multi_gen_files(putty, compound, minyld, OUTPUT_TYPES, OUTPUT_DIR, TIMEWAIT)
            else:
