...
import logging
import os
from transformer_extract_reaction import SpeciesExtractor, ReactionExtractor, MechanismWriter

os.chdir(os.path.dirname(__file__))  # Sets working dir to script's location
...
def main():
    ...
    TARGET_REACTANT = "APINENE" #"ISOPRENE"
    TARGET_LABEL = "API" #"ISO"
    TARGET_GEN = "Multi"
    TARGET_MINYLD = "0.001"

    ref_path = "MechGen_Base.m"
    input_path = f"MG-{TARGET_REACTANT}_Rxnfile_{TARGET_MINYLD}.dat"
    output_path = f"MechGen_{TARGET_REACTANT}_{TARGET_MINYLD.replace('0.','')}.m"
    ...
    replacements = {
        "RAD_": f"RAD_{TARGET_LABEL}",
        "PRO_": f"PRO_{TARGET_LABEL}",
        "ORG_": f"ORG_{TARGET_LABEL}",
    }

    for old, new in replacements.items():
        output_contents = output_contents.replace(old, new)
