...
class MechanismWriter:
    ...
    @staticmethod
    def write_mech(file_handle, 
                   compounds_list, reaction_list,
                    name_replacement, 
                    target_reactant, target_minyld, 
                    target_label=None):
        """
        ...
        Parameters:
            ...
            name_replacement (dict): Dictionary for name replacements.
            target_reactant (str): Target reactant name.
            target_minyld (float): Minimum yield threshold.
            target_label (str): Label for the target species.
        """
        NUMBER_COMPOUND = 6
       ...
       # Write header
        ...
        # Apply name_replacements to compounds list
        compounds_list = [name_replacement.get(comp, comp) for comp in compounds_list]

        # Then apply prefix replacement if target_label is set
        if target_label:
            def prefix_replace(name):
                for prefix in ["PRO", "ORG", "RAD"]:
                    if name.startswith(prefix):
                        return name.replace(prefix, f"{target_label}{prefix}", 1)
                return name
            compounds_list = [prefix_replace(comp) for comp in compounds_list]
        ...
        for reaction in reaction_list:
            ...
            for old, new in name_replacement.items():
                reaction = reaction.replace(old, new)

            # Then apply prefix renaming (PRO → ISOPRO etc.) if needed
            if target_label:
                for prefix in ["PRO", "ORG", "RAD"]:
                    reaction = re.sub(rf'\b{prefix}(\w*)\b', rf'{target_label}{prefix}\1', reaction)
