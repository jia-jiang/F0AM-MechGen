function [SpRates, ax] = PlotRatesSave(Spname,ExpDate,RunID,S,n2plot,varargin)
% function [SpRates, ax] = PlotRatesSave(Spname,RunID,exp_date,S,n2plot,varargin)
...
%               Default: 1
%
%           PlotRatesSave(...,'plotme',value)
%               Specifies whether to generate plot (1) or not
%               Default: 1
%
%           PlotRatesSave(...,'parent',axis)
...
%    ax:                axis handle to figure created. Spname
%
...
% 20240926 JJ       Added input of RunID,exp_date for saving the plot, and
%                   handel the species names with '_'
...
varInfo = {...
    ...
    'unitT'         's'         {'s','min','hr','day'};
    'scale'         1           [];...
    'ptype'         'fill'      {'fill','bar','line'};...
    'sumEq'         0           [0 1];...
    'plotme'        1           [0 1];...
    'parent'        0           [];         
    'save_path'     'D:\Google Drive\Model\F0AM\Output\figure\'   [];...
    };
...
if plotme
    ...
    %Plot decorations
    if ~isempty(Pnames)
        Pnames = regexprep(Pnames, '^A_', ''); 
        Pnames = regexprep(Pnames, '_', '-'); 
        legend(ax1,Pnames,'Location','NorthEast')
    end
    
    if ~isempty(Lnames)
        Lnames = regexprep(Lnames, 'A_', '');  
        Lnames = regexprep(Lnames, '_', '-'); 
    ...
    xticks(minT:stepT:maxT); % Adjust the step size 
    ...
    saveas(new_figure, strcat(save_path,'/Rates_', Spname, '_' , ExpDate, RunID,'.fig'))
